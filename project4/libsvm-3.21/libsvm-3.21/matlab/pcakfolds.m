function [splitfile,classlabel,difflabel,accur,taccprint]= pcakfolds( kvalue,sample,newproj,method,knnval,svmthd,exflg,dimen )
...
for j=1:kfold
    ...
    if strcmp(exflg,'A')
        [U,S,V]=svd(traindata(2:end,1:end),'econ');
        newU=U(:,1:dimen);
        trdata=newU'*traindata(2:end,1:end);
        testdata=newU'*testdata;
        traindata=vertcat(trainlabel,trdata);
    end
